...
Status: Phase 9 escalation implementation and local technical verification are complete as of 2026-09-14. The [verification and human review package](docs/superpowers/phase9-verification.md) records 491 backend tests, 38 web tests, 12 connected scenarios, standalone browser and container checks. Project-owner acceptance remains pending. PostgreSQL owns saved drafts, recipients, dispatch, responses, responsibility and escalation. This repository remains public; this phase does not change GitHub settings.
...
Phase 4 provides a fictional practitioner directory, CSV import adapter, validation/preview, and searchable directory UI. Phase 5 adds protected typed simulation alert drafting and SBAR confirmation. Phase 6 adds manual fictional-recipient selection, protected approved-message content, exact review, and idempotent human confirmation that creates an identifier-only outbox item. Phase 7 adds a Development/Test-only simulation worker, typed local channel adapters, deterministic provider-event scenarios, bounded retry, lease recovery, and safe delivery-status projection. Phase 8 adds simulation practitioner responses, call-unit requests, operator resolve/cancel actions, a safe manual-fallback placeholder, and a read-only operator status surface. Real providers, hospital connectors, SCIM, Graph, FHIR, AI features, Entra SSO, production identity, external callbacks, production escalation, and Phase 10 remain out of scope.

## Phase 9 implementation boundary

The [Phase 9 design](docs/superpowers/specs/2026-09-07-phase-9-escalation-design.md), [implementation plan](docs/superpowers/plans/2026-09-07-phase-9-escalation.md), and [escalation architecture](docs/architecture/escalation.md) define the authorized Development/Test-only work. Exact policy ID/version, step timing and every future backup/channel must be shown before human confirmation and bound by a plan revision. Changed policy/directory/on-call evidence rejects confirmation; legacy alerts without exact snapshots remain ineligible.

PostgreSQL UTC time and durable leases drive execution. The worker may activate only preconfirmed backups through the existing identifier-only outbox. Acceptance/lifecycle/escalation share alert-before-run locking; decline/unavailable signals expedite a step once; inactive backups fail visibly with manual fallback and no substitute. Acknowledgement does not accept responsibility. Pause/Resume preserve durable remaining delay. The browser only submits human actions and reads durable status.

This authorization supersedes the prior Phase 8.5 stop instruction for Phase 9 only. Earlier reports and unrecorded approvals remain historical; Phase 9 is technically verified and awaits human acceptance. All production timing, authority, eligibility and fallback choices remain `REQUIRES_HOSPITAL_DECISION`. Stop after Phase 9 for human review.
...
Phase 8 explicitly links the fictional Riley user to one organization-scoped practitioner. Practitioner-only `/api/v1/my-alerts` routes expose the approved message for addressed Active alerts and record SecureMessage opening, acknowledgement, call-unit request, or one terminal accepted/declined/unavailable disposition with idempotent transactions. Acceptance creates a separate exact-version responsibility assignment. The Operator, Administrator, ClinicalSupervisor, Auditor and SystemAdministrator live-reader policy permits `/api/v1/alerts/{id}/live` to return a safe refreshed projection without clinical content. Resolve/cancel and Phase 9 pause/resume use the distinct lifecycle-operator policy, which excludes Auditor, and require their server-side preconditions. Delivery failures show a manual-fallback placeholder marked `REQUIRES_HOSPITAL_DECISION`; no production fallback route is configured. Both surfaces derive identity and organization from the server principal, fail closed outside Development/Test where applicable, and expose no encrypted storage values, contact endpoints, or raw provider data.
