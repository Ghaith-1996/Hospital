...
The repository is at the Phase 8 simulation-only doctor-response and closed-loop compliance-correction gate. Phase 7 is the reviewed and pushed predecessor at commit `45f4024`; no Phase 7 tag is recorded in the repository. Phase 8 includes the explicit organization-scoped user-to-practitioner link, practitioner inbox/detail, opened/acknowledged/call-unit/accepted/declined/unavailable response semantics, responsibility assignment, safe operator live projection, simulation-only resolve/cancel actions, and a manual-fallback placeholder. Do not begin Phase 9 or add real providers, external callbacks, escalation automation, production resolution/transfer policy, hospital directory connections, SCIM, Graph, FHIR, Entra/production identity, AI, or other behavior outside the Phase 8 simulation scope. Production meanings for lifecycle, responsibility, escalation, and fallback remain `REQUIRES_HOSPITAL_DECISION`.
