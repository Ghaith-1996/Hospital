...
The repository is at the Phase 6 review gate: simulation-only recipient selection and final review are implemented locally and await project-owner review. Phase 0 through Phase 5 have been approved and tagged, and the Phase 6 plan was approved for inline execution in this task. Until the project owner approves Phase 6, make only review corrections to manual recipient selection from the fictional directory, selected-recipient summaries, operator-approved message content, exact review, idempotent authenticated confirmation, sanitized audit, atomic creation of an identifier-only outbox item, and the existing fictional directory boundary. Do not process outbox items or add workers, retries, delivery attempts, provider adapters, hospital directory connections, SCIM, Graph, FHIR, production identity, or any other Phase 7 behavior.
