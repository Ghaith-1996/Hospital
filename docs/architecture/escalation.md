...
The separate `SimulationEscalation` worker configuration is disabled by default and guarded to Development/Test. Its host uses a fresh scope per bounded scheduling iteration. At this implementation slice the host schedules only; activation and outbox processing remain the following tasks. The recognized rule strings are centralized in `DemoEscalationSemantics`; arbitrary policy text fails review/confirmation, and unsupported stored plans cannot be scheduled or claimed. Supported confirmed snapshots remain authoritative after later mutable policy edits.
...
Task 6 adds the unregistered `EscalationRunProcessor.ProcessClaimAsync` operation. It requires an awaited enqueue callback that stages dispatch work in the same scoped DbContext and transaction before advancement; a no-op callback is test-only. Task 7 supplies the outbox callback before any worker activation is enabled. Stop and failure paths never invoke it. The existing unique RecipientActivated event links each new selection to the exact run/version/step; immutable plan membership supplies its confirmed practitioner/role/channel evidence without a second provenance store.
