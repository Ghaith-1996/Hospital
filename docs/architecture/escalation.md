...
Task 8 supplies POST `/api/v1/alerts/{id}/escalation/pause` and `/resume` in Development/Test, using the existing lifecycle-operator role policy. Both accept `ExpectedVersion` and `ReasonCode`: Pause allows `OperatorReview` or `ManualCoordination`; Resume allows `ReadyToResume`. Keys contain 1–100 visible ASCII characters. One shared idempotency namespace hashes the action, organization, actor, alert, version and reason; reuse across actions or changed payloads returns a safe 409. Stored result references contain only an opaque append-only event ID. A retained replay returns the original state, reason and occurrence time even after a later override or lifecycle stop; the live projection remains authoritative for current state.

The live GET reads its alert, exact plan/run, activated selections, delivery facts, original and escalation outboxes, responsibility and timeline in one repeatable-read transaction. Its refresh time comes from PostgreSQL `clock_timestamp()`. The additive `escalation` object reports exact eligibility, policy/version/revision, run state, bounded current/next/total step presentation, database-owned next evaluation, durable remaining pause seconds, terminal outcome/safe reason/failure, independent escalation-outbox state/failure, and deterministic identifier-only timeline. Completed exhaustion clamps the displayed current step to the final confirmed step and reports no next step; the immutable run cursor is not presented as an unapproved future step.

Each grouped practitioner now includes per-selection channel/source evidence. Only an activated `EscalationPolicy` selection with its matching immutable `RecipientActivated` event receives run/step/policy/revision provenance; manual selections and missing legacy bindings never receive invented provenance. Future confirmation snapshots are not selections and remain absent from live recipients and practitioner inbox access. Live-reader roles can inspect this evidence, while `CanPause`/`CanResume` are also masked by the existing lifecycle-operator authorization policy and exact durable state, so an Auditor never receives an actionable override flag. Original dispatch outbox status remains separate from escalation queue/failure status. Exhaustion or processing/delivery failure requests manual fallback only while responsibility/lifecycle remains unresolved; accepting responsibility later does not rewrite terminal run history.

Fresh controls require an eligible exact confirmed plan and existing nonterminal run, and recheck active exact-version responsibility and lifecycle under the alert-then-run locks. Pause clears an owned worker lease and persists remaining delay; Resume restores it from PostgreSQL UTC time. No control creates a missing or legacy run. The additive `Phase9EscalationOverrideReason` migration stores bounded typed reasons with the immutable event; historical reasons remain nullable without invented backfill. Non-GUID HTTP correlation values become a stable opaque UUID shared by audit and timeline.

Recipient open/response and lifecycle mutations now read PostgreSQL time after taking the shared alert lock. Controls read it after both alert and run locks. If database time precedes committed alert, selection, run, response, responsibility or delivery evidence, a fresh mutation returns a safe retry conflict without partial effects or timestamp clamping. Retained successful replay is checked first and remains available despite later future-dated evidence. Committed exact-version responsibility always blocks fresh controls, even before the worker records Stopped or while its timestamp appears future.
...
Final persistence also defers if PostgreSQL time moves behind the callback's staged run update. It rolls back even previously saved event/outbox/run effects, clears EF tracking and retains the original persisted acquisition. A fresh scope can retry that acquisition while valid; a running worker that has discarded it waits for normal durable lease recovery. The regression test proves zero committed effects and safe retry, without clamping clocks or weakening domain timestamp checks.
