...
Meter: CriticalAlerts.Platform. Implemented counters: criticalalerts.alert.confirmations, criticalalerts.dispatch.retries, criticalalerts.delivery.events, criticalalerts.responses, criticalalerts.directory.imports, criticalalerts.outbox.processed, criticalalerts.dispatch.failures, criticalalerts.audit.queries, criticalalerts.escalation.steps, criticalalerts.escalation.stopped, criticalalerts.escalation.exhausted and criticalalerts.escalation.failures. These count committed audit operations, not individual send attempts or recipients. The sole tag is the closed operation vocabulary. The initial proposed attempts/failure counters and duration histograms are omitted because the commit observer does not measure those quantities truthfully. Emit only after transaction commit; retries/replays must not claim another durable success. Metrics represent local process observations, not a durable accounting ledger. Restore exercise durations are separately measured by the script.
