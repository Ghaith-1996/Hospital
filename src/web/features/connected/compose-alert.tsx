...
export function ComposeAlert({ alertId }: { alertId: string }) {
  ...
  return <><PageHeader title="Compose Alert" description="Review source, SBAR, critical fields and the secure message." />
    ...
    {query.loading ? <Loading /> : query.data && <ComposeForm key={alertId} draft={query.data} onSaved={query.setData} onConflict={setConflict} />}
  </>;
}
...
function ComposeForm({ draft, onSaved, onConflict }: { draft: api.AlertDraft; onSaved(value: api.AlertDraft): void; onConflict(error: unknown): void }) {
  ...
  async function command(run: () => Promise<api.AlertDraft>, next?: () => void) {
    ...
    try { const value = await run(); if (!normalizedDirty) releaseDirty(); onSaved(value); next?.(); }
      ...
      if (api.isAlertApiError(failure) && failure.status === 409) {
        setError(null); onConflict(failure); setStale(true);
      }
  ...
  async function discardAndLoad() {
    ...
    try {
      ...
      releaseDirty(); onSaved(current); setStale(false); setError(null); onConflict(null);
  ...
  return <div className="new-alert-layout">
    <div className="new-alert-form">
      ...
      {stale && <section className="error-panel"><p>Your unsaved edits are retained below. Discard them explicitly to load and review the current server version.</p><button type="button" disabled={busy} onClick={() => void discardAndLoad()}>Discard local edits and load server version</button></section>}
      ...
      <div className="form-actions"><button type="button" className="button-secondary" disabled={busy} onClick={() => { setContent(contentFrom(draft)); setMessage(draft.approvedMessage ?? ""); setNormalized({}); }}>Discard unsaved edits</button>
        <button type="button" disabled={busy || stale || contentDirty || messageDirty || normalizedDirty} onClick={() => void command(() => api.submitAlertDraft(draft.alertId, draft.draftVersion), () => router.push(`/alerts/${draft.alertId}/review`))}>Submit for exact review</button></div>
    </div>
