...
export function AuditEvents() {
  ...
  return <div className="page-stack">
    ...
    <form className="detail-card" aria-label="Audit filters" onSubmit={event => { event.preventDefault(); load({ filters: { ...filters }, cursors: [null], revision: request.revision + 1 }); }}>
      <div className="detail-grid">
        ...
        {([["action", "Action", auditActions], ["outcome", "Outcome", auditOutcomes], ["resourceType", "Resource type", auditResourceTypes]] as const).map(([key, label, choices]) =>
          <div className="filter-field" key={key}><label htmlFor={`audit-${key}`}>{label}</label><select id={`audit-${key}`} value={filters[key] ?? ""} onChange={event => change(key, event.target.value)}><option value="">All</option>{choices.map(value => <option key={value} value={value}>{value}</option>)}</select></div>)}
    ...
    <p role="status" aria-label="Audit query status" ref={status} tabIndex={-1}>{loading ? "Loading audit events…" : error ? "Audit query failed." : `Page ${request.cursors.length} · ${page?.events.length ?? 0} events`}</p>
    ...
    {page && !loading && (page.events.length === 0 ? <p>No audit events match these filters.</p> : <div className="table-wrap">
      <table className="directory-table" aria-label="Audit events"><thead><tr>{["Time (UTC)", "Action", "Resource type", "Resource identifier", "Actor type", "Outcome", "Correlation ID", "Safe metadata"].map(label => <th scope="col" key={label}>{label}</th>)}</tr></thead>
